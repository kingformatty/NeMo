    ...
    def forward_streaming_step_1(
        ...
        if streaming_level == 'emb':
            ...
            if True: #only sync streaming
                # if streaming_state.fifo is None or left_context == 0:
                if streaming_state.fifo.shape[1] == 0:
                    self.query_embs = chunk_pre_encode_embs[:, :query_len-1]
                    chunk_pre_encode_embs = chunk_pre_encode_embs[:, query_len-1:]
                else:
                    chunk_pre_encode_embs = chunk_pre_encode_embs[:, -tokens_per_chunk-1:-1]
                spkcache_fifo_chunk_pre_encode_embs = self.sortformer_modules.concat_embs(
                    [self.query_embs, streaming_state.spkcache, streaming_state.fifo, chunk_pre_encode_embs], dim=1, device=self.device
                )
                spkcache_fifo_chunk_pre_encode_lengths = (
                    query_len +
                    streaming_state.spkcache.shape[1] +
            ...
            # import ipdb; ipdb.set_trace()
            query_spkcache_fifo_chunk_fc_encoder_embs, query_spkcache_fifo_chunk_fc_encoder_lengths = self.frontend_encoder(
                processed_signal=spkcache_fifo_chunk_pre_encode_embs,
                processed_signal_length=spkcache_fifo_chunk_pre_encode_lengths,
                bypass_pre_encode=True
            )
            query_spkcache_fifo_chunk_preds = self.forward_infer(
                emb_seq=query_spkcache_fifo_chunk_fc_encoder_embs,
                emb_seq_length=query_spkcache_fifo_chunk_fc_encoder_lengths
            )
            import pickle; import numpy as np
            query_spkcache_fifo_chunk_preds = self.sortformer_modules.apply_mask_to_preds(
                query_spkcache_fifo_chunk_preds, query_spkcache_fifo_chunk_fc_encoder_lengths
            )
            streaming_state, chunk_preds = self.sortformer_modules.streaming_update_1(
                streaming_state=streaming_state,
                chunk=chunk_pre_encode_embs,
                preds=query_spkcache_fifo_chunk_preds,
                lc=round(left_offset / self.encoder.subsampling_factor),
                rc=math.ceil(right_offset / self.encoder.subsampling_factor),
                query_len = query_len
            )
        ...
        self.spkcache_fifo_chunk_preds = query_spkcache_fifo_chunk_preds
        self.spkcache_fifo_chunk_preds_lengths = query_spkcache_fifo_chunk_fc_encoder_lengths
        return streaming_state, total_preds
