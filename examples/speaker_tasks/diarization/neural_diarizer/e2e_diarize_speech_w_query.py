    ...
    with open(manifest, 'r') as inp_file:
        ...
        for line in lines:
            ...
            meta = {
                ...
                'query_audio_filepath': dic['query_audio_filepath'],
                'query_offset': dic['query_offset'],
                'query_duration': dic['query_duration'],
                'query_speaker_id': dic['query_speaker_id'],
            ...
            uniqname += meta['query_speaker_id']
...
def convert_pred_mat_to_segments(
    ...
    for sample_idx, (uniq_id, audio_rttm_values) in enumerate(audio_rttm_map_dict.items()):
        ...
        if audio_rttm_values.get("uniq_id", None) is not None:
            ...
            uniq_id += audio_rttm_values['query_speaker_id']
...
def timestamps_to_pyannote_object_w_query(
    ...
    if rttm_file is not None and os.path.exists(rttm_file):
        ...
        if consider_query_in_eval:
            ...
            query_offset = audio_rttm_values.get('query_offset',None)
            query_duration = audio_rttm_values.get('query_duration',None)
